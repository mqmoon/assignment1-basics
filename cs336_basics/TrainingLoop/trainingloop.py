...
def get_args():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='checkpoints_lr2em3', help='Directory to save checkpoints.')
    ...
    parser.add_argument('--max_iters', type=int, default=6000, help='Total number of training iterations.')
    parser.add_argument('--learning_rate', type=float, default=2e-3, help='Maximum learning rate.')
    
    # Logging and Saving
    parser.add_argument('--val_interval', type=int, default=100, help='How often to evaluate on validation set.')
    parser.add_argument('--save_interval', type=int, default=500, help='How often to save a checkpoint.')
    parser.add_argument('--val_iters', type=int, default=10, help='Number of batches to average for evaluation loss.')
...
def estimate_loss(model, train_data, val_data, args):
    ...
    for split_name, split_data in [('train', train_data), ('val', val_data)]:
        losses = torch.zeros(args.val_iters)
        for k in range(args.val_iters):
...
def main():
    ...
    for iter_num in range(start_iter + 1, args.max_iters + 1):
        ...
        loss.backward()
        clip_grad(parameters=model.parameters(), max_norm=1.0)
        lr = get_lr_cosine_schedule(t=iter_num, alpha_max=args.learning_rate, alpha_min=0.05*args.learning_rate,
                                    Tw=0.05*args.max_iters, Tc=args.max_iters)
        ...
        dt = t1 - t0
        t0 = t1
        print(f"iter {iter_num}: learning rate {lr:.6f}, loss {loss.item():.4f}, time {dt*1000:.2f}ms")
